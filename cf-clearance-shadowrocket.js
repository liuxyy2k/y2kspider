...
// Cloudflare Clearance 脚本 for Shadowrocket / Loon
// Shadowrocket 配置：见同目录 Shadowrocket 配置示例
//
...
// 构造 iOS Safari UA。Shadowrocket 没有 Loon 的 $loon.systemVersion，
// 因此这里优先使用缓存的过盾 UA；仅首次没有缓存时使用兜底版本。
CF.buildSafariUA = function () {
  var version = CF.CONFIG.FALLBACK_UA_VERSION_DOTTED;
...
CF.notify = function (subtitle, content, attach) {
  try {
    if (attach) {
      // Shadowrocket/Loon 均可能存在通知参数差异，先尝试带跳转参数
      try {
...
// attach 跳转到「触发盾的原始请求 URL」，让用户点击后回到触发请求的地址，方便重新验证。
...
// ============ 入口 ============
// Shadowrocket 与 Loon 均提供 $request / $response / $done / $persistentStore / $notification，
// 本版本去除对 $loon 的依赖，因此可用于 Shadowrocket。
